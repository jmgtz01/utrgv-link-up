...
class Computer(models.Model):
    ...
    x = models.DecimalField(
        max_digits=5, decimal_places=2, default=0)  # left %
    y = models.DecimalField(
        max_digits=5, decimal_places=2, default=0)  # top  %
    status = models.CharField(
        max_length=12, choices=COMPUTER_STATUSES, default="available")

    reserved_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="reserved_computers")
...
class StudyRoom(models.Model):
    ...
    status = models.CharField(
        max_length=12, choices=ROOM_STATUSES, default="available")

    reserved_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="reserved_rooms")
...
class Event(models.Model):
    ...
    manager = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name='managed_events')
    email_address = models.EmailField('Email Address')
    description = models.TextField(blank=True)
    attendees = models.ManyToManyField(Student, blank=True)
    # image_name = models.CharField(max_length=60)
    image = models.ImageField(upload_to='events-list/', blank=True, null=True)
...
class Reservation(models.Model):
    ...
    class Meta:
        indexes = [
            models.Index(
                fields=["resource_type", "resource_id", "start", "end"]),
